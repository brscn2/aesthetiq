...
class ProductLoader:
    ...
    async def load_products(
        self, products: List[Dict[str, Any]], retailer_id: Optional[str] = None
    ) -> int:
        ...
        for product in products:
            try:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to generate embedding for {item_data.get('productUrl')}: {e}"
                    )
                ...
                logger.error(
                    f"Failed to load product {product.get('name', 'unknown')}: {e}"
                )
    ...
    async def _prepare_item_data(
        self, product: Dict[str, Any], retailer_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        if any(
            word in category_str
            for word in ["TOP", "SHIRT", "BLOUSE", "SWEATER", "JACKET", "COAT"]
        ):
            category = Category.TOP
        elif any(
            word in category_str
            for word in ["BOTTOM", "PANT", "JEAN", "SKIRT", "SHORT"]
        ):
            category = Category.BOTTOM
        elif any(
            word in category_str for word in ["FOOTWEAR", "BOOT", "SNEAKER", "SANDAL"]
        ):
            category = Category.SHOE
        elif any(
            word in category_str
            for word in ["ACCESSORY", "BAG", "HAT", "BELT", "SCARF"]
        ):
