...
async def save_item_feedback(
    ...
    try:
        ...
        coll = get_collection(
            settings.MONGODB_DB_WARDROBE,
            "item_feedback"  # Feedback collection
        )
...
async def list_disliked_wardrobe_items(
    ...
    try:
        ...
        wardrobe_coll = get_collection(settings.MONGODB_DB_WARDROBE, settings.MONGODB_COLLECTION_WARDROBE)
        ...
        ).sort("updated_at", -1).skip(safe_offset).limit(capped_limit)
