...
def _doc_to_item(doc: Dict[str, Any]) -> CommerceItem:
    ...
    return CommerceItem(
        ...
        brandId=(
            str(normalized_doc["brandId"]) if normalized_doc.get("brandId") else None
        ),
...
async def filter_commerce_items(
    filters: Optional[CommerceFilters] = None, *, limit: int = 100
) -> List[CommerceItem]:
    """
    ...
    docs = await db.find_commerce_items(
        filters=filter_dict, limit=limit, use_retail_collection=False
    )
    ...
    *,
    ...
    for doc in docs:
        ...
        if item.seasonalPaletteScores:
            palette_scores_dict = item.seasonalPaletteScores.model_dump(
                exclude_none=True
            )
