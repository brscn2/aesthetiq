...
async def find_commerce_items(
    ...
    *,
    ...
    collection_name = (
        settings.MONGODB_COLLECTION_RETAIL
        if use_retail_collection
        else settings.MONGODB_COLLECTION_COMMERCE
    )
...
async def get_commerce_item(
    item_id: str, use_retail_collection: bool = False
...
async def upsert_commerce_item(item_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = await coll.update_one(
            {"productUrl": product_url}, update_op, upsert=True
        )
    except Exception as e:
        # Handle duplicate key errors - try to update existing document instead
        if "duplicate key" in str(e).lower() or "E11000" in str(e):
            logger.warning(
                f"Duplicate key error for {product_url}, attempting to update existing document: {e}"
            )
            # Find existing document by productUrl and update it
            result = await coll.update_one({"productUrl": product_url}, update_op)
            if result.matched_count == 0:
                # If still can't find it, try without sku constraint
                update_op_no_sku = {
                    "$set": {k: v for k, v in update_data.items() if k != "sku"}
                }
                result = await coll.update_one(
                    {"productUrl": product_url}, update_op_no_sku, upsert=True
                )
        else:
            raise
