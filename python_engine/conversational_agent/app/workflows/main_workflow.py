...
class ClarificationExtraction(BaseModel):
    ...
    occasion: Optional[str] = Field(
        None,
        description="Event type: interview, business, casual, party, wedding, date, formal",
    )
    budget: Optional[str] = Field(
        None, description="Budget level: budget, mid-range, luxury"
    )
    max_price: Optional[int] = Field(
        None, description="Maximum price in dollars if mentioned"
    )
    color: Optional[str] = Field(None, description="Color preference mentioned")
    style: Optional[str] = Field(
        None,
        description="Style preference: classic, modern, minimalist, bold, elegant, casual, sporty",
    )
    ...
    additional_context: Optional[str] = Field(
        None, description="Any other relevant details from the response"
    )
...
def route_after_analysis(
    state: ConversationState,
) -> Literal["approve", "approve_with_feedback", "refine", "clarify", "error"]:
    ...
    if iteration >= max_iterations:
        logger.warning(
            f"Max iterations ({max_iterations}) reached, routing to approve anyway"
        )
        return "approve"
...
def route_after_output_guardrails(
    state: ConversationState,
) -> Literal["safe", "unsafe"]:
...
async def input_guardrails_node(state: ConversationState) -> Dict[str, Any]:
    ...
    if result.is_safe:
        logger.info(
            f"Input guardrails passed (provider: {result.provider}, risk: {result.risk_score:.2f})"
        )
        if result.warnings:
            logger.warning(f"Input guardrail warnings: {result.warnings}")
    else:
        logger.warning(
            f"Input guardrails blocked (provider: {result.provider}, risk: {result.risk_score:.2f}, warnings: {result.warnings})"
        )

    # Return updated state with sanitized message if it was modified
    return {
        "message": (
            result.sanitized_content if result.sanitized_content != message else message
        ),
        "metadata": metadata,
    }
...
- Size
- Clothing category (TOP, BOTTOM, FOOTWEAR, OUTERWEAR, DRESS, ACCESSORY)
...
Only extract values that are clearly stated or strongly implied."""
        ...
        if extraction:
            ...
            logger.info(
                f"LLM extracted filters: {extraction.model_dump(exclude_none=True)}"
            )

    except Exception as e:
        logger.warning(
            f"LLM-based clarification extraction failed, using keyword fallback: {e}"
        )
    ...
    logger.info(
        f"Updated filters after clarification (LLM={llm_extraction_used}): {updated_filters}"
    )
...
async def output_guardrails_node(state: ConversationState) -> Dict[str, Any]:
    ...
    if result.is_safe:
        logger.info(
            f"Output guardrails passed (provider: {result.provider}, risk: {result.risk_score:.2f})"
        )
        if result.warnings:
            logger.warning(f"Output guardrail warnings: {result.warnings}")
    else:
        logger.warning(
            f"Output guardrails blocked (provider: {result.provider}, risk: {result.risk_score:.2f}, warnings: {result.warnings})"
        )
    ...
    return {
        "final_response": (
            ...
            if result.sanitized_content != response
            else response
        ),
        "metadata": metadata,
    }
...
async def error_response_node(state: ConversationState) -> Dict[str, Any]:
    ...
    if not metadata.get("input_safe", True):
        ...
        response = (
            "I'm having trouble finding exactly what you're looking for. "
...
async def run_workflow(
    ...
    log_msg = (
        f"Running workflow for user {user_id}, session {session_id}, trace {trace_id}"
    )
    if is_clarification:
    ...
    try:
        ...
        if needs_clarification:
            trace_metadata["clarification_question"] = final_state.get(
                "clarification_question", ""
            )
        ...
        if workflow_status == "awaiting_clarification":
            logger.info(
                f"Workflow paused for clarification, question sent ({response_len} chars)"
            )
        else:
...
async def run_workflow_streaming(
    ...
    try:
            ...
            if event_type == "on_chain_end" and event.get("data", {}).get("output"):
                ...
                if isinstance(output, dict):
                    ...
                    if "final_response" in output:
                        logger.debug(
                            f"Captured final state with response from node: {event_name}"
                        )
        ...
        if final_state is None or not final_state.get("final_response"):
            logger.warning(
                "Final state not captured from streaming or missing final_response, running sync fallback"
            )
            try:
                ...
                logger.error(
                    f"Fallback workflow invocation also failed: {fallback_error}"
                )
        ...
        if workflow_status == "awaiting_clarification":
            logger.info(
                f"Streaming workflow paused for clarification ({response_len} chars)"
            )
        else:
            logger.info(
                f"Streaming workflow completed with response length: {response_len}"
            )
