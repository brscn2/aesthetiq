...
def observe(name: Optional[str] = None):
    ...
    def decorator(func):
        ...
        # If Langfuse not available, return function unchanged
        @wraps(func)
...
class LangfuseService:
    ...
    Langfuse v3 API methods:
    - create_event(name, input, output, metadata, ...)
    - create_score(name, value, trace_id, comment, ...)
    - create_trace_id() -> str
    
    Features:
        - Trace LLM calls and agent workflows
    ...
    def start_trace(
        ...
        Returns a trace context dict that should be passed to child operations.
        ...
        Returns:
            Trace context dict with trace_id and trace_context, or None if disabled
        """
        ...
        try:
            ...
            # Create TraceContext for nesting observations
            trace_context = _TraceContext(trace_id=trace_id) if _TraceContext else None
            
            # Log start event linked to trace
            ...
            # Return context for child operations
            return {
                "trace_id": trace_id,
                "trace_context": trace_context,
                "name": name,
    ...
    def end_trace(
        ...
        Args:
            trace_context: Trace context dict from start_trace()
            output: Final output of the traced operation
            metadata: Additional metadata
        """
    ...
    def log_generation(
        ...
        """
        Log a single LLM generation as an event linked to a trace.
    ...
    def score_trace(
        ...
        Args:
            trace_context: Trace context dict from start_trace()
            name: Score name (e.g., "user_rating", "quality")
    ...
    def get_langchain_callback(self):
        ...
        Example:
            >>> callback = langfuse.get_langchain_callback()
