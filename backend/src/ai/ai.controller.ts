...
} from './dto/remove-background.dto';
...
export class AiController {
  ...
  @ApiResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        data: {
          type: 'string',
          description: 'Base64 encoded image with transparent background',
        },
        error: { type: 'string' },
      },
    },
  ...
  async removeBackground(
    ...
    try {
      const result = await this.backgroundRemovalService.removeBackground(
        dto.imageBase64,
      );
      return {
        success: true,
