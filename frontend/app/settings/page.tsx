  ...
  return (
    <DashboardLayout>
      <div className="flex h-full flex-col bg-background lg:flex-row">
        ...
        {/* Left Side: Vertical Navigation - Drawer on Mobile */}
        <div
          className={`fixed inset-y-0 left-0 z-50 w-80 transform border-r border-border bg-sidebar transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0 ${
            navOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
          }`}
        >
          <div className="flex h-full flex-col p-4 sm:p-6">
            <SettingsNav onClose={() => setNavOpen(false)} />
          </div>
        </div>
